...
```{code-cell} ipython3
# As MLP running time is too long, the code is masked. 
...
```{code-cell} ipython3
train_X = train_X.reshape(-1, 28,28, 1) # to make a transform from 784 vectors to a 28*28*1 matrix
...
```{code-cell} ipython3
# As CNN running time is too long, the code is masked. 
